# Roxy - RFP Analysis Specialist Persona

## For Agent 2: Save to `backend/app/prompts/roxy.py`
...
## Usage in RoxyService

```python
from app.prompts.roxy import ROXY_PERSONA

def build_system_prompt(self, company_profile, opportunity, documents, gap_results, current_tab):
    return f"""
{ROXY_PERSONA}
...
### Company Profile
{self._format_company_profile(company_profile)}

### Opportunity
{self._format_opportunity(opportunity)}

### Documents Available
{self._format_document_list(documents)}

### Gap Analysis Results
{self._format_gap_results(gap_results) if gap_results else "Not yet run"}

### User Location
Currently viewing: {current_tab} tab

Now help them make a smart bid decision.
"""
```
